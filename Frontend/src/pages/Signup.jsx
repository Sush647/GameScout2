...
function Signup() {
    ...
    const signup=(e)=>{
        if(username==""||email==""||password==""){
            ...
            e.preventDefault()
            firebase.signup(username,email,password);
        }
    }
  ...
  return (
    <div className='flex justify-center items-center h-screen  bg-center bg-no-repeat bg-cover mt-2' style={{backgroundImage:`url(${bg})`}}>
     <form className='backdrop-blur-lg ring-1 ring-black shadow-md shadow-black w-full max-w-fit sm:max-w-96 md:max-w-[28rem] h-fit py-5 px-8 rounded-xl'>
        ...
        <h1 className='text-white font-semibold text-2xl w-full text-center'>Signup</h1>

        <div className='flex flex-col items-center justify-center gap-3 mt-3'>

        <div className='flex flex-col gap-1 w-full'>
            <label htmlFor="Username" className='text-white font-semibold text-xl'>Username</label>
            <input type="text" placeholder='enter you username' className='outline-none bg-white p-2 w-full rounded-md'  value={username} onChange={(e)=>{setusername(e.target.value)}} />
        </div>
        ...
        <div className='w-full flex flex-col gap-3 mt-3'>
            <button className='w-full p-2 bg-gray-800 text-white font-semibold text-lg rounded-md active:bg-gray-600 cursor-pointer' onClick={signup}>Signup</button>
